...
describe("astroRenderer plugin", () => {
	...
	describe("transform hook", () => {
		...
		it("should NOT intercept non-.astro files", async () => {
			...
			expect(
				await transform?.call({} as any, code, "/path/to/file.ts", options),
			).toBeNull();
			expect(
				await transform?.call({} as any, code, "/path/to/file.tsx", options),
			).toBeNull();
			expect(
				await transform?.call({} as any, code, "/path/to/file.js", options),
			).toBeNull();
			expect(
				await transform?.call({} as any, code, "/path/to/file.jsx", options),
			).toBeNull();
		});
	});
